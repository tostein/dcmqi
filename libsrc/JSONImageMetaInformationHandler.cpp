...
namespace dcmqi {
	...
	JSONImageMetaInformationHandler::JSONImageMetaInformationHandler(string jsonInput)
		: JSONMetaInformationHandlerBase(jsonInput) {
	}

	JSONImageMetaInformationHandler::~JSONImageMetaInformationHandler() {
	...
	void JSONImageMetaInformationHandler::setClinicalTrialCoordinatingCenterName(const string &coordinatingCenterName) {
		this->coordinatingCenterName = coordinatingCenterName;
	}

	void JSONImageMetaInformationHandler::setContentCreatorName(const string &creatorName) {
		this->contentCreatorName = creatorName;
	}

	void JSONImageMetaInformationHandler::setClinicalTrialSeriesID(const string &seriesID) {
		this->clinicalTrialSeriesID = seriesID;
	}

	void JSONImageMetaInformationHandler::setClinicalTrialTimePointID(const string &timePointID) {
		this->clinicalTrialTimePointID = timePointID;
	}

	void JSONImageMetaInformationHandler::read() {
		try {
			...
			this->seriesDescription = this->metaInfoRoot.get("SeriesDescription", "Image").asString();
	...
	bool JSONImageMetaInformationHandler::write(string filename) {
		ofstream outputFile;
	...
	string JSONImageMetaInformationHandler::getJSONOutputAsString() {
		if (this->segmentsAttributesMappingList.size() == 0)
			return string();
	...
	Json::Value JSONImageMetaInformationHandler::createAndGetSegmentAttributes() {
	...
	SegmentAttributes *JSONImageMetaInformationHandler::createAndGetNewSegment(unsigned labelID) {
		for (vector<map<unsigned, SegmentAttributes*> >::const_iterator vIt = this->segmentsAttributesMappingList.begin();
			vIt != this->segmentsAttributesMappingList.end(); ++vIt) {
			for (map<unsigned, SegmentAttributes*>::const_iterator mIt = vIt->begin(); mIt != vIt->end(); ++mIt) {
				SegmentAttributes *segmentAttributes = mIt->second;
	...
	void JSONImageMetaInformationHandler::readSegmentAttributes() {
